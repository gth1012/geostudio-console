...
export default function BatchesPage() {
  ...
  // bulk-upload 모달 드래그 상태
  const [bulkModalPos, setBulkModalPos] = useState({ x: 0, y: 0 });
  const [isBulkDragging, setIsBulkDragging] = useState(false);
  const bulkDragOffset = useRef({ x: 0, y: 0 });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['batches'] }); setDeleteTarget(null); toast.show('작업이 삭제되었습니다', 'success'); },
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.show(`${ids.length}개 작업이 삭제되었습니다`, 'success');
    } catch (err: any) {
  ...
  const addImagesWithDuplicateCheck = useCallback((newImages: BatchRow[]) => {
    setRows(prev => {
      ...
      if (duplicateCount > 0) {
        toast.show(`중복 이미지 ${duplicateCount}개 제외`, 'error');
      }
      return [...prev, ...unique];
    });
  }, [toast]);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (files.length === 0) {
      toast.show('이미지 파일만 업로드 가능합니다', 'error');
      return;
    }
    processFiles(files);
  }, [processFiles, toast]);
  ...
  // bulk-upload 모달 드래그 핸들러
  const handleBulkMouseDown = (e: React.MouseEvent) => {
    const tag = (e.target as HTMLElement).tagName;
    if (['INPUT','TEXTAREA','SELECT','BUTTON','A','SVG','PATH'].includes(tag)) return;
    setIsBulkDragging(true);
    bulkDragOffset.current = { x: e.clientX - bulkModalPos.x, y: e.clientY - bulkModalPos.y };
  };
  const handleBulkMouseMove = (e: React.MouseEvent) => {
    if (!isBulkDragging) return;
    setBulkModalPos({ x: e.clientX - bulkDragOffset.current.x, y: e.clientY - bulkDragOffset.current.y });
  };
  const handleBulkMouseUp = () => { if (isBulkDragging) setIsBulkDragging(false); };

  const handleBulkFileSelect = (files: File[]) => {
    const allowed = ['image/jpeg', 'image/jpg', 'image/png'];
    const valid = files.filter(f => allowed.includes(f.type));
    const invalid = files.length - valid.length;
    if (invalid > 0) toast.show(`사용할 수 없는 파일 ${invalid}개 제외 (jpg/png만 사용)`, 'error');

    setBulkFiles(prev => {
      const existingNames = new Set(prev.map(f => f.name));
      const unique = valid.filter(f => !existingNames.has(f.name));
      const dup = valid.length - unique.length;
      if (dup > 0) toast.show(`중복 파일 ${dup}개 제외`, 'error');
      const merged = [...prev, ...unique];
      if (merged.length > MAX_BULK_FILES) {
        toast.show(`최대 ${MAX_BULK_FILES}개까지 업로드 가능합니다`, 'error');
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-row items-center gap-3 mb-6">
        <button onClick={() => { setModalPos({ x: 0, y: 0 }); setShowModal(true); }} className="px-4 py-2 bg-status-yellow-dim text-status-yellow rounded-lg hover:bg-status-yellow/20 text-sm font-medium transition-all">작업 생성</button>

        <button
          onClick={() => { setBulkModalPos({ x: 0, y: 0 }); setShowBulkUploadModal(true); }}
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex justify-center gap-1">
                      <button onClick={() => navigate(`/batches/${b.batch_id}`)}
                        className="w-16 px-1 py-1 text-xs rounded transition-all bg-status-yellow-dim text-status-yellow hover:bg-status-yellow/20">상세/자산생성</button>
      ...
      {!filteredBatches?.length && (
        <div className="mt-4 px-6 py-5 bg-status-purple-dim border border-status-purple/30 rounded-lg text-center">
          ...
          <p className="text-sm text-txt-secondary"><button onClick={() => { setModalPos({ x: 0, y: 0 }); setShowModal(true); }} className="text-status-yellow font-semibold hover:underline">[작업 생성]</button>{' '}에서 시리즈 선택 후 작업을 만드세요.</p>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
              <div className="space-y-4">
                ...
                {rows.length > 0 && (
                  <div className="space-y-3">
                    <label className="block text-xs text-txt-secondary">이미지 목록 ({rows.length}개)</label>
                    {rows.map((row, i) => (
              ...
              <div className="flex gap-2 mt-6">
                ...
                <button type="submit" disabled={isCreating || rows.length === 0} className="flex-1 px-4 py-2.5 bg-status-yellow-dim text-status-yellow rounded-lg font-medium hover:bg-status-yellow/20 transition-all disabled:opacity-50">
                  {isCreating ? `생성 중.. (${createProgress.current}/${createProgress.total})` : `총 발행량 ${rows.reduce((sum: number, r: any) => sum + (Number(r.supply) || 0), 0)}개`}
                </button>
              </div>
            </form>
          </div>
      ...
      {showBulkUploadModal && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-start justify-start p-4"
          onMouseMove={handleBulkMouseMove}
          onMouseUp={handleBulkMouseUp}
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              {!bulkUploading && !bulkDone && (
                ...
                >
                  ...
                  <p className="text-xs text-txt-muted mt-1">jpg, png 사용 · 최대 300장 · 파일당 10MB</p>
                </div>
              )}
              ...
              {bulkFiles.length > 0 && !bulkUploading && !bulkDone && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => setBulkFiles([])} className="text-xs text-status-red hover:underline">전체 삭제</button>
                  </div>
              ...
              {bulkUploading && (
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-txt-muted text-center">100장 단위 청크 처리 중입니다. 잠시만 기다려주세요.</p>
                </div>
              )}
