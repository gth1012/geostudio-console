...
export default function BatchesPage() {
  ...
  const handleBulkPrint = async () => {
    ...
    try {
      ...
      toast.show(`${ids.length}개 코드생성이 시작되었습니다`, 'success');
    } catch (err: any) {
      toast.show(err.response?.data?.message || '일부 코드생성 실패', 'error');
    } finally {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-row items-center gap-3 mb-6">
        ...
        {selectedIds.size > 0 && (
          <button onClick={() => setShowBulkPrintModal(true)} className="px-4 py-2 bg-status-yellow-dim text-status-yellow rounded-lg hover:bg-status-yellow/20 text-sm font-medium transition-all">
            코드생성 ({selectedIds.size}개)
          </button>
        )}
      </div>
      ...
      {batches?.length > 0 && batches.every((b: any) => b.status === 'PRINTED') && (
        <div className="mt-4 px-6 py-5 bg-status-purple-dim border border-status-purple/30 rounded-lg text-center">
          ...
          <p className="text-sm text-txt-secondary">다음 작업은{' '}<button onClick={() => window.location.hash = '/exports'} className="text-status-yellow font-semibold hover:underline">[자산 출고]</button>에서 진행하세요.</p>
        </div>
      )}
      {!batches?.length && (
        <div className="mt-4 px-6 py-5 bg-status-purple-dim border border-status-purple/30 rounded-lg text-center">
          <p className="text-base font-semibold text-txt-primary mb-1">작업이 없습니다.</p>
          <p className="text-sm text-txt-secondary"><button onClick={() => { setModalPos({ x: 0, y: 0 }); setShowModal(true); }} className="text-status-yellow font-semibold hover:underline">[시리즈 선택]</button>{' '}에서 시리즈 선택 후 작업을 이어가세요.</p>
        </div>
      )}
      {/* 삭제 확인 모달 */}
      {deleteTarget && (
      ...
      {showBulkPrintModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-geo-card border border-geo-border rounded-xl w-full max-w-md p-6">
            <h3 className="text-base font-semibold text-txt-primary mb-2">일괄 코드생성</h3>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowBulkPrintModal(false)} className="flex-1 px-4 py-2.5 bg-status-purple text-white rounded-lg font-medium hover:bg-status-purple/80 transition-all disabled:opacity-50">원본이미지 {rows.length}개</button>
              <button onClick={handleBulkPrint} disabled={isBulkPrinting} className="flex-1 px-4 py-2.5 bg-status-yellow-dim text-status-yellow rounded-lg font-medium hover:bg-status-yellow/20 transition-all disabled:opacity-50">
                {isBulkPrinting ? '생성 중...' : `코드생성 (${selectedIds.size}개)`}
              </button>
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start justify-start p-4" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
          <div className="bg-geo-card border border-geo-border rounded-xl w-full max-w-md flex flex-col cursor-move select-none" style={{ maxHeight: '85vh', transform: `translate(${modalPos.x}px, ${modalPos.y}px)` }} onMouseDown={handleMouseDown}>
            <div className="bg-geo-main px-4 py-3 border-b border-geo-border rounded-t-xl flex-shrink-0 flex justify-between items-center"><div className="w-6" />
              ...
              </select><button type="button" onClick={() => { setShowModal(false); setSeriesId(''); setRows([]); }} className="w-6 h-6 flex items-center justify-center text-txt-muted hover:text-txt-primary transition-all text-lg">✕</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
              ...
              <div className="flex gap-2 mt-6">
                ...
                <button type="submit" disabled={isCreating || rows.length === 0} className="flex-1 px-4 py-2.5 bg-status-yellow-dim text-status-yellow rounded-lg font-medium hover:bg-status-yellow/20 transition-all disabled:opacity-50">
                  {isCreating ? `생성 중... (${createProgress.current}/${createProgress.total})` : `총 생성 이미지 ${rows.reduce((sum: number, r: any) => sum + (Number(r.supply) || 0), 0)}개`}
                </button>
              </div>
            </form>
          </div>
