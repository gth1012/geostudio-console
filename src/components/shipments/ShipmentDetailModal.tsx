...
export default function ShipmentDetailModal({ shipmentId, onClose }: ShipmentDetailModalProps) {
  ...
  const getCapsuleStatus = () => {
    ...
    const statuses = shipment.shipmentAssets.map(sa => sa.asset?.geocapsule_status).filter(Boolean);
  ...
  const getCapsuleStatusStyle = (status: string) => {
    const map: Record<string, string> = {
      READY: 'text-status-green',
      PENDING: 'text-status-yellow',
      FAILED: 'text-status-red',
    };
    return map[status] || 'text-txt-secondary';
  };

  const getCapsuleStatusLabel = (status: string) => {
    const map: Record<string, string> = {
      READY: 'READY',
      PENDING: '생성 중',
      FAILED: '생성 실패',
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 space-y-3">
          ...
          {/* 그룹 3: 인증 캡슐 상태 - 출고확정 후에만 표시 */}
          {shipment.status === 'SHIPPED' && capsuleStatus && (
            <div className="bg-geo-main border border-geo-border rounded-lg px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-base text-status-purple">인증 캡슐</span>
                <span className={`text-base font-semibold ${getCapsuleStatusStyle(capsuleStatus)}`}>
                  ● {getCapsuleStatusLabel(capsuleStatus)}
                </span>
              </div>
            </div>
          )}
